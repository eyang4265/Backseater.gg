...
class AggregateCommands(commands.Cog):
    ...
    @discord.option(
        "summoner", description="Primary player's Game Name", required=False
    )
    @discord.option("tag", description="Primary player's tagline", required=False)
    @discord.option(
        "user", description="Primary user (defaults to you)", required=False
    )
    async def duo(self, ctx, teammate, server=None, summoner=None, tag=None, user=None):
        """Handle duo."""
        log_command(
            ctx, server=server, summoner=summoner, tag=tag, user=user, teammate=teammate
        )
        await ctx.defer()
        target = await target_for(ctx, server, summoner, tag, user, include_icon=False)
        ...
        if target is None or teammate_account is None:
            await ctx.respond(
                embed=make_embed("Both players must resolve to tracked accounts.")
            )
            return
