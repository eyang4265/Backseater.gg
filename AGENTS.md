...
### Slash-command registration must remain sync-safe

Every Discord slash command must place all required options before all optional
options. Discord rejects the entire bulk command sync when a required option
follows an optional one, leaving existing commands with stale schemas. Before
finishing any public command change, run the network-free registration test and
verify startup logs contain `Synchronized application commands`.
