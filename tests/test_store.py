...
from bot_app.store import PlayerState, dedupe_tail, load_live_game_state, save_live_game_state


class DedupeTailTests(unittest.TestCase):
    def test_keeps_the_newest_ids_in_order(self) -> None:
        # Regression: the previous implementation sliced a set, so trimming
        # could discard the newest ids and re-announce old matches.
...
class PlayerStateTests(unittest.TestCase):
    ...
    def test_reads_the_current_format(self) -> None:
        state = PlayerState.from_json(
            {
                "matches": ["NA1_1"],
                "solo": {"tier": "GOLD", "rank": "II", "lp": 40, "wins": 5, "losses": 4},
                "flex": None,
            }
        )
    ...
    def test_round_trips_through_json(self) -> None:
        state = PlayerState(matches=["NA1_1"], ranks={420: RankSnapshot("IRON", "IV", 1)})
        self.assertEqual(PlayerState.from_json(state.to_json()).matches, state.matches)
        self.assertEqual(PlayerState.from_json(state.to_json()).ranks[420], RankSnapshot("IRON", "IV", 1))
...
class LiveGameStateTests(unittest.TestCase):
    def test_state_round_trip(self) -> None:
        ...
        with TemporaryDirectory() as directory, patch("bot_app.store.LIVE_GAME_STATE_PATH", Path(directory) / "live.json"):
