...
def _frame(timestamp, positions, stats=None, events=()):
    ...
    return {"timestamp": timestamp, "participantFrames": participant_frames, "events": list(events)}
...
class SoloKillTests(unittest.TestCase):
    def _timeline(self, events):
        ...
        return MatchTimeline(
            {"info": {"frames": [_frame(0, far_away), _frame(60_000, far_away, events=events)]}}
        )
...
class KillLocationTests(unittest.TestCase):
    def _timeline(self, events):
        ...
        return MatchTimeline(
            {"info": {"frames": [_frame(0, positions), _frame(60_000, positions, events=events)]}}
        )
    ...
    def test_kills_without_a_position_are_dropped(self) -> None:
        placeless = {"type": "CHAMPION_KILL", "timestamp": 61_000, "killerId": 1, "victimId": 2}
        self.assertEqual(self._timeline([placeless]).champion_kills(), [])

    def test_other_event_types_are_ignored(self) -> None:
        turret = {"type": "BUILDING_KILL", "timestamp": 61_000, "position": {"x": 5, "y": 5}}
...
class LaneDiffTests(unittest.TestCase):
    def _timeline(self, last_timestamp):
        stats = {
            1: {"totalGold": 5_000, "xp": 6_000, "minionsKilled": 80, "jungleMinionsKilled": 4},
            2: {"totalGold": 4_000, "xp": 5_500, "minionsKilled": 70, "jungleMinionsKilled": 0},
        }
        positions = {1: (0, 0), 2: (10, 10)}
        return MatchTimeline(
            {"info": {"frames": [_frame(0, positions, stats), _frame(last_timestamp, positions, stats)]}}
        )
    ...
    def test_level_diff_comes_from_each_side_of_the_curve(self) -> None:
        # 6,000 XP is level 9 and change; 5,500 is a little under half a level
        # behind it — not the flat number a raw XP gap would suggest.
        diff = self._timeline(600_000).lane_diff_at(1, 2, 600_000)
    ...
    def test_frame_outside_the_tolerance_is_rejected(self) -> None:
        # Game ended before the requested minute mark.
...
class LaneDiffSeriesTests(unittest.TestCase):
        ...
        return MatchTimeline(
            {
                "info": {
                    "frames": [
                        _frame(minute * 60_000, positions, stats) for minute in range(frame_count)
                    ]
                }
            }
        )
    ...
    def test_marks_step_by_five_minutes_to_the_end_of_the_game(self) -> None:
        # 22 frames = minutes 0..21, so 5/10/15/20 have data and 25 does not.
    ...
    def test_short_game_stops_early(self) -> None:
        self.assertEqual([minute for minute, _ in self._timeline(8).lane_diff_series(1, 2)], [5])
    ...
    def test_a_mark_missing_frame_data_does_not_end_the_series(self) -> None:
        ...
        # Drop the opponent from the 10-minute frame only.
        del timeline._frames[10]["participantFrames"]["2"]
        self.assertEqual([minute for minute, _ in timeline.lane_diff_series(1, 2)], [5, 15, 20])
...
#: Riot's published cumulative XP per level, 1 through 20. The curve is
#: generated rather than spelled out in timeline.py, so this is the reference
#: that says the generator is right.
_CUMULATIVE_XP = [
...
class LevelCurveTests(unittest.TestCase):
    def test_the_generated_curve_matches_the_published_table(self) -> None:
        for level, cumulative in enumerate(_CUMULATIVE_XP, start=1):
            with self.subTest(level=level):
                self.assertEqual(level_at_xp(cumulative, TOP_LANE_MAX_LEVEL), float(level))

    def test_each_level_costs_100_more_than_the_last(self) -> None:
        steps = [_CUMULATIVE_XP[i] - _CUMULATIVE_XP[i - 1] for i in range(1, len(_CUMULATIVE_XP))]
    ...
    def test_thresholds_land_on_whole_levels(self) -> None:
        ...
        self.assertEqual(level_at_xp(280), 2.0)  # 280 XP for the first level
        self.assertEqual(level_at_xp(660), 3.0)  # 380 more for the second
        self.assertEqual(level_at_xp(18_360), float(MAX_LEVEL))

    def test_progress_between_levels_is_fractional(self) -> None:
        self.assertEqual(level_at_xp(470), 2.5)  # halfway from 280 to 660

    def test_the_curve_stretches_as_levels_get_expensive(self) -> None:
        # The same 190 XP that bought half of level 2 barely dents level 17.
    ...
    def test_top_lane_keeps_levelling_past_the_usual_cap(self) -> None:
        # 20,665 XP is level 18 for everyone else and level 19 up top.
        ...
        self.assertEqual(level_at_xp(30_000, TOP_LANE_MAX_LEVEL), float(TOP_LANE_MAX_LEVEL))
...
@unittest.skipUnless((_SAMPLES / "sample_timeline.json").exists(), "sample match data is missing")
    ...
    def test_every_frame_matches_riots_own_level(self) -> None:
        ...
        for frame in timeline["info"]["frames"]:
            for participant_id, participant_frame in frame["participantFrames"].items():
                ...
                if expected is None:
                    continue
                derived = int(level_at_xp(participant_frame.get("xp", 0), caps[participant_id]))
                self.assertEqual(
                    derived,
                    expected,
...
class LaneLineTests(unittest.TestCase):
    ...
    def test_nothing_is_padded(self) -> None:
        series = [(minute, LaneDiff(gold=1, xp=0, levels=0.1, cs=1)) for minute in (5, 40)]
