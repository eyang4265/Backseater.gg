...
class RankValueTests(unittest.TestCase):
    def test_scale_is_monotonic_across_divisions_and_tiers(self) -> None:
        """Verify that scale is monotonic across divisions and tiers."""
        ladder = [
            ("IRON", "IV", 0),
            ("IRON", "I", 99),
            ("BRONZE", "IV", 0),
    ...
    def test_apex_tiers_share_one_lp_pool(self) -> None:
        """Verify that apex tiers share one lp pool."""
        self.assertEqual(
            rank_value("MASTER", "", 300), rank_value("CHALLENGER", "", 300)
        )
        self.assertEqual(rank_value("MASTER", "", 0), APEX_THRESHOLD)

    def test_unranked_and_unknown_tiers_have_no_value(self) -> None:
        """Verify that unranked and unknown tiers have no value."""
    ...
    def test_value_round_trips_below_master(self) -> None:
        """Verify that value round trips below master."""
        for tier, division, lp in [
            ("GOLD", "II", 47),
            ("EMERALD", "IV", 0),
            ("IRON", "I", 99),
        ]:
            with self.subTest(tier=tier, division=division):
                self.assertEqual(
                    value_to_rank(rank_value(tier, division, lp)), (tier, division, lp)
                )

    def test_value_above_the_apex_threshold_reports_flat_lp(self) -> None:
        """Verify that value above the apex threshold reports flat lp."""
        self.assertEqual(
            value_to_rank(rank_value("GRANDMASTER", "", 412)), ("MASTER", "", 412)
        )

    def test_value_to_rank_of_none_is_none(self) -> None:
        """Verify that value to rank of none is none."""
        self.assertIsNone(value_to_rank(None))


class SnapshotTests(unittest.TestCase):
    def test_unranked_snapshot_is_not_ranked(self) -> None:
        """Verify that unranked snapshot is not ranked."""
    ...
    def test_state_round_trip_keeps_the_legacy_field_names(self) -> None:
        """Verify that state round trip keeps the legacy field names."""
    ...
    def test_from_state_rejects_non_dicts(self) -> None:
        """Verify that from state rejects non dicts."""
...
class LpChangeTests(unittest.TestCase):
    def test_gain_and_loss_are_signed(self) -> None:
        """Verify that gain and loss are signed."""
    ...
    def test_promotion_across_tiers_is_measured_on_one_scale(self) -> None:
        """Verify that promotion across tiers is measured on one scale."""
        self.assertEqual(
            lp_change(
                RankSnapshot("GOLD", "I", 88), RankSnapshot("PLATINUM", "IV", 10)
            ),
            "+22 LP",
        )

    def test_missing_or_unranked_sides_produce_no_label(self) -> None:
        """Verify that missing or unranked sides produce no label."""
...
class RankQueueTests(unittest.TestCase):
    ...
    def test_label_names_the_queue_being_shown(self) -> None:
        """Verify that label names the queue being shown."""
